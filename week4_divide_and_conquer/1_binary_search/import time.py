...
bread_board=box(length=6,width=2,height=0.2,color=color.white,opacity=0.25)
Arduino=box(pos=vector(-2,.125,0),length=2,width=1.5,height=0.1,color=color.blue,opacity=0.25)
...
def Quanterion(splitPacket):
    Q=[]
    Q.append(float(splitPacket[0])*toRad)
    Q.append(float(splitPacket[1])*toRad)
    Q.append(float(splitPacket[2])*toRad)
    Q.append(float(splitPacket[3])*toRad)
    q0 = Q[0]
    q1 = Q[1]
    q2 = Q[2]
    q3 = Q[3]
    ...
    obj.up=vector(final[0],final[1],final[2])
    obj.axis=vector(final[3],final[4],final[5])
